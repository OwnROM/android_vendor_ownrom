...
function setup_vendor() {
    ...
    export OWNROM_ROOT="$3"
    if [ ! -d "$OWNROM_ROOT" ]; then
        echo "\$OWNROM_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$OWNROM_ROOT/$OUTDIR" ]; then
        mkdir -p "$OWNROM_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$OWNROM_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$OWNROM_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$OWNROM_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2018 The OwnROM Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The OwnROM Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2014-$YEAR The OwnROM Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The OwnROM Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The OwnROM Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$OWNROM_ROOT"/vendor/ownrom/build/tools/smali/baksmali.jar
        export SMALIJAR="$OWNROM_ROOT"/vendor/ownrom/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$OWNROM_ROOT"/vendor/ownrom/build/tools/"$HOST"/vdexExtractor
    fi
    ...
    if [ ! -f "$OWNROM_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$OWNROM_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$OWNROM_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$OWNROM_ROOT"/vendor/ownrom/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$OWNROM_ROOT"/"$OUTDIR"/radio
